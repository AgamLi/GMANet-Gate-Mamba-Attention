...
from .VMRNN_self.vmamba import VSSBlock, SS2D  
...
class VSB(VSSBlock):
    ...
    def forward(self, x, hx=None):
        ...
        if hx is not None:
            hx = self.ln_1(hx) #  (B,L,C)
            x = torch.cat((x, hx), dim=-1) #  (B,L,2C)
            x = self.linear(x) # (B,L,C)
        x = x.view(B, H, W, C) # (B,L,C)-->(B,H,W,C)

        x = self.drop_path(self.self_attention(x)) #   (B,H,W,C)-->(B,H,W,C)

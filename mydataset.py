...
class JointTransform2D:
    ...
    Args:
        crop: tuple describing the size of the random crop. If bool(crop) evaluates to False, no crop will
            be taken.
        p_flip: float, the probability of performing a random horizontal flip_rotation.
    ...
    def __call__(self, image, mask):
        ...
        # random horizontal flip_rotation
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # image = resize_image_itk(image, (224, 224, 3))
        image = sitk.GetArrayFromImage(image)
        image = image.transpose((1, 2, 0))

        mask = sitk.ReadImage(os.path.join(label_path, filename + '.mha'))
        # mask = resize_image_itk(mask, (224, 224))
